...
/**
 * Router that contains information about both route matching orders and HTTP request methods. Routes are devided into 3 sections: "first", "last", and "other". Routes in "first" are matched first, then in "other", then in "last".
 * <h3>Create router</h3> Route targets can be any type. In the below example, targets are classes:
 *
 * <pre>
 * {
 * 	&#64;code
 * 	Router<Class> router = new Router<Class>().GET("/articles", IndexHandler.class).GET("/articles/:id", ShowHandler.class).POST("/articles", CreateHandler.class).GET("/download/:*", DownloadHandler.class) // ":*" must be the last token
 ...
 * </pre>
 *
 * <h3>Match with request method and URI</h3> Use {@link #route(HttpMethod, String)}. From the {@link RouteResult} you can extract params embedded in the path and from the query part of the request URI.
 * <h3>404 Not Found target</h3> Use {@link #notFound(Object)}. It will be used as the target when there's no match.
 *
 * <pre>
 * router.notFound(My404Handler.class);
 * </pre>
 *
 * <h3>Create reverse route</h3> Use {@link #path(HttpMethod, Object, Object...)} or {@link #path(Object, Object...)}:
 *
 * <pre>
 * {@code
 ...
 * </pre>
 *
 * <h3>Allowed methods</h3> If you want to implement <a href="https://en.wikipedia.org/wiki/Hypertext_Transfer_Protocol#Request_methods">OPTIONS</a> or <a href="https://en.wikipedia.org/wiki/Cross-origin_resource_sharing">CORS</a>, you can use {@link #allowedMethods(String)}. For {@code OPTIONS *},
 * use {@link #allAllowedMethods()}.
 * @param <T>
 */
public class Router<T>
{
	...
	/**
	 * @return the fallback target for use when there's no match at {@link #route(HttpMethod, String)}.
	 */
	public T notFound()
	{
		return notFound;
	...
	/**
	 * @return Returns the number of routes in this router.
	 */
	public int size()
	{
	...
	/**
	 * Add route to the "first" section. A path can only point to one target. This method does nothing if the path has already been added.
	 * @param method
	 * @param path
	 * @param target
	 * @return
	...
	/**
	 * Add route to the "other" section. A path can only point to one target. This method does nothing if the path has already been added.
	 * @param method
	 * @param path
	 * @param target
	 * @return
	...
	/**
	 * Add route to the "last" section. A path can only point to one target. This method does nothing if the path has already been added.
	 * @param method
	 * @param path
	 * @param target
	 * @return
	...
	public RouteResult<T> route(HttpMethod method, String uri)
	{
		...
		if (notFound != null)
		{
			...
			return new RouteResult<>(notFound, new HashMap<String, String>(), decoder.parameters());
		}
	...
	/**
	 * Returns allowed methods for a specific URI. For {@code OPTIONS *}, use {@link #allAllowedMethods()} instead of this method.
	 * @param uri
	 * @return
	 */
	...
	/**
	 * @return all methods that this router handles. For {@code OPTIONS *}.
	 */
	...
	/**
	 * Given a target and params, this method tries to do the reverse routing and returns the path. The params are put to placeholders in the path. The params can be a map of {@code placeholder name -> value} or ordered values. If a param doesn't have a placeholder, it will be put to the query part
	 * of the path.
	 * @param method
	 * @param target
	 * @param params
	...
	public String toString()
	{
		...
		List<String> paths = new ArrayList<>(numRoutes);
	 ...
	 * @param <T>
	 */
	private static <T> void aggregateRoutes(String method, Map<Path, T> routes, List<String> accMethods, List<String> accPaths, List<String> accTargets)
	{
	...
	/**
	 * Helper for toString; for example, returns "io.netty.example.http.router.HttpRouterServerHandler" instead of "class io.netty.example.http.router.HttpRouterServerHandler"
	 * @param target
	 * @return
	 */

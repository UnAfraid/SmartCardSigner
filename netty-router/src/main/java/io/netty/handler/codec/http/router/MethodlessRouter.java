...
/**
 * Router that contains information about route matching orders, but doesn't contain information about HTTP request methods. Routes are devided into 3 sections: "first", "last", and "other". Routes in "first" are matched first, then in "other", then in "last".
 * @param <T>
 */
final class MethodlessRouter<T>
{
	...
	/**
	 * Returns the "other" router; routes in this router will be matched after those in the "first" router, but before those in the "last" router.
	 * @return
	 */
	...
	/**
	 * Adds route to the "first" section. A path can only point to one target. This method does nothing if the path has already been added.
	 * @param path
	 * @param target
	 * @return
	 */
	...
	/**
	 * Adds route to the "other" section. A path can only point to one target. This method does nothing if the path has already been added.
	 * @param path
	 * @param target
	 * @return
	 */
	...
	/**
	 * Adds route to the "last" section. A path can only point to one target. This method does nothing if the path has already been added.
	 * @param path
	 * @param target
	 * @return
	 */
	...
	{
		return first.anyMatched(requestPathTokens) || other.anyMatched(requestPathTokens) || last.anyMatched(requestPathTokens);
	}

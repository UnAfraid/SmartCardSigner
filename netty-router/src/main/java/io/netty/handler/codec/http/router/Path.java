...
/**
 * The path can contain constants or placeholders, example: {@code constant1/:placeholder1/constant2/:*}. {@code :*} is a special placeholder to catch the rest of the path (may include slashes). If exists, it must appear at the end of the path. The path must not contain URL query, example:
 * {@code constant1/constant2?foo=bar}. The path will be broken to tokens, example: {@code ["constant1", ":variable", "constant2", ":*"]}
 */
final class Path
{
	...
	/**
	 * The path must not contain URL query, example: {@code constant1/constant2?foo=bar}. The path will be stored without slashes at both ends.
	 * @param path
	 */
	...
	/**
	 * {@code params} will be updated with params embedded in the path. This method signature is designed so that {@code pathTokens} and {@code params} can be created only once then reused, to optimize for performance when a large number of paths need to be matched.
